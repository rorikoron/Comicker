...
pub struct TypstWrapperWorld {
    ...
    /// Tauri window for progress reporting.
...
impl TypstWrapperWorld {
    pub fn new(root: String, source: String, window: Option<tauri::Window>, total: usize) -> Self {
...
impl TypstWrapperWorld {
    ...
    fn file(&self, id: FileId) -> FileResult<FileEntry> {
        ...
        // Increment progress and emit update if this is a local image
        if id.package().is_none() && id.vpath().as_rootless_path().extension().and_then(|e| e.to_str()) == Some("png") {
            let current = self.current.fetch_add(1, std::sync::atomic::Ordering::SeqCst) + 1;
            if let Some(window) = &self.window {
                use tauri::Emitter;
                #[derive(serde::Serialize, Clone)]
                struct ProgressPayload { current: usize, total: usize }
                let _ = window.emit("merge-progress", ProgressPayload { current, total: self.total });
            }
        }
